...
const ChaptersList = ({ items, onEdit, onReorder }: ChaptersListProps) => {
  ...
  // macete pra resolver problema de hydration
  useEffect(() => {
    setIsMounted(true)
  }, [])
            ...
            {chapters.map((chapter, index) => (
              ...
              >
                {(provided) => (
                  <div 
                    className={cn(
                      "flex items-center gap-x-2 bg-slate-200 border-slate-300 border mb-4 text-sm rounded-md",
                  ...
                  >
                    <div 
                      className={cn(
                        "px-2 py-3 border-r border-r-slate-200 hover:bg-slate-300 transition",
